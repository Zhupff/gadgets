...
class StackBlur : Blur {
    ...
    private val blurScope = CoroutineScope(SupervisorJob() + Dispatchers.IO)

    override fun sync(config: BlurConfig): Bitmap? {
        config.config()
        return blur(config)
    }

    override fun async(config: BlurConfig, callback: Blur.AsyncCallback) {
        config.config()
        blurScope.launch {
            val output = blur(config)
    ...
    private fun blur(config: BlurConfig): Bitmap? {
        val bitmap = config.bitmap
        ...
        for (y in 0 until height) {
            ...
            for (x in 0 until width) {
                pixels[index[0]] =
                    (0xFF000000.toInt() and pixels[index[0]]) or (div[rgbSum[0]] shl 16) or (div[rgbSum[1]] shl 8) or div[rgbSum[2]]
