...
class AudioPlayer : IMediaPlayer {
    ...
    private val TAG = "AudioPlayer(${hashCode()})"
    ...
    override fun prepare(source: String, callback: Runnable?): Boolean {
    ...
    private inner class MediaPlayerListeners :
        ...
        override fun onPrepared(mp: MediaPlayer?) {
            ...
            val state = this@AudioPlayer.state as? MediaState.PREPARING ?: return
            this@AudioPlayer.sendMessage(MediaState.PREPARED(state.source, state.callback))
        }
        override fun onBufferingUpdate(mp: MediaPlayer?, percent: Int) {
            Log.d(TAG, "onBufferingUpdate($percent)")
            this@AudioPlayer.sendMessage(MediaEvent.BUFFER_UPDATE(percent / 100F))
        }
        override fun onCompletion(mp: MediaPlayer?) {
            Log.d(TAG, "onCompletion")
            this@AudioPlayer.sendMessage(MediaEvent.COMPLETION)
        }
        override fun onSeekComplete(mp: MediaPlayer?) {
            Log.d(TAG, "onSeekComplete")
            val state = this@AudioPlayer.state as? MediaState.SEEKING ?: return
            this@AudioPlayer.sendMessage(MediaState.SOUGHT(state.toMs, state.callback))
        }
        override fun onInfo(mp: MediaPlayer?, what: Int, extra: Int): Boolean {
            Log.d(TAG, "onInfo($what, $extra)")
            this@AudioPlayer.sendMessage(MediaEvent.PLAYER_INFO(what, extra))
            return true
        }
        override fun onError(mp: MediaPlayer?, what: Int, extra: Int): Boolean {
            Log.d(TAG, "onError($what, $extra)")
            this@AudioPlayer.sendMessage(MediaEvent.PLAYER_ERROR(what, extra))
            return true
        }
    }
}