...
public final class Version {
  ...
  BitMatrix buildFunctionPattern() {
    ...
    BitMatrix bitMatrix = new BitMatrix(dimension, dimension);

    // Top left finder pattern + separator + format
    bitMatrix.region(0, 0, 9, 9);
    // Top right finder pattern + separator + format
    bitMatrix.region(dimension - 8, 0, 8, 9);
    // Bottom left finder pattern + separator + format
    bitMatrix.region(0, dimension - 8, 9, 8);
    ...
    for (int x = 0; x < max; x++) {
      ...
      for (int y = 0; y < max; y++) {
        if ((x != 0 || (y != 0 && y != max - 1)) && (x != max - 1 || y != 0)) {
          bitMatrix.region(alignmentPatternCenters[y] - 2, i, 5, 5);
        }
    ...
    bitMatrix.region(6, 9, 1, dimension - 17);
    // Horizontal timing pattern
    bitMatrix.region(9, 6, dimension - 17, 1);

    if (versionNumber > 6) {
      // Version info, top right
      bitMatrix.region(dimension - 11, 0, 3, 6);
      // Version info, bottom left
      bitMatrix.region(0, dimension - 11, 6, 3);
    }
